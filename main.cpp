...
void move_apple() {
    // Ensure that the apple isn't being moved inside the snake
    int number_of_open_spaces = (GRID_SIZE * GRID_SIZE) - snake_len;

    if (number_of_open_spaces == 0) {
        // TODO: If apple can't be placed anywhere where the snake is not, we end the game.
        // game_state->quit = true;
        assert(number_of_open_spaces == 0);
        return;
    }

    bool grid_state[GRID_SIZE][GRID_SIZE];
    for (int i = 0; i < GRID_SIZE; i++) {
        for (int j = 0; j < GRID_SIZE; j++) {
            grid_state[i][j] = false;
        }
    }
    // apple *free_cells_array = (apple *) malloc(number_of_open_spaces * sizeof(apple));
    apple free_cells_array[GRID_SIZE*GRID_SIZE];

    Snake *tracker = head;

    while (tracker != NULL) {
        grid_state[tracker->x][tracker->y] = true;
        tracker = tracker->next;
    }

    int current_free_cell_index = 0;
    for (int i = 0; i < GRID_SIZE; i++) {
        for (int j = 0; j < GRID_SIZE; j++) {
            if (grid_state[i][j] != true) {
                free_cells_array[current_free_cell_index] = { i, j};
                current_free_cell_index++;
            }
        }
    }

    int random_free_cell = uniform_random(0, number_of_open_spaces - 1);

    printf("Random cell index: %d", random_free_cell);
    apple next_apple = free_cells_array[random_free_cell];

    printf("Apple spawning at (%d,%d)\n", next_apple.x, next_apple.y);
    current_apple->x = next_apple.x;
    current_apple->y = next_apple.y;
}
